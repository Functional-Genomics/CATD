...
run_CDSeq<-function(T, C){
	RESULTS<-CDSeq::CDSeq(bulk_data = T, cell_type_number = length(unique(marker_distrib$CT)), mcmc_iterations = 1000,
							  cpu_number=10,block_number=6,gene_subset_size=1000, reference_gep = C)$estProp
	return(RESULTS)
}
run_MuSiC<-function(T, C){
	require(MuSiC)
...
run_deconvSeq<-function(T, C, phenoDataC){
...
run_SCDC<-function(T, C, phenoDataC){
